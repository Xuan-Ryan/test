...
/* --------------------------------------------------------------------------
 * driect controls //add luke
 */
int uvc_dr_ctrl_query(struct uvc_video_chain *chain,
	struct v4l2_direct_control *dctrl, int set)
{
	...
	if(dctrl->wLength> 4096)
		return -EFAULT;

    if(dctrl->mode == 1){
		memset(data,0,12);
		memcpy(data,&chain->dev->intfnum,4);
		
		list_for_each_entry(entity, &chain->entities, chain) {
			if(UVC_ENTITY_TYPE(entity) == UVC_VC_PROCESSING_UNIT){ 
				//printk("UVC_VC_PROCESSING_UNIT \n");
				memcpy(&data[5],&entity->id,1);
				memcpy(&data[9],entity->processing.bmControls,entity->processing.bControlSize);
			}
			if(UVC_ENTITY_TYPE(entity) == UVC_ITT_CAMERA){ 
				//printk("UVC_ITT_CAMERA \n");
				memcpy(&data[4],&entity->id,1);
				memcpy(&data[6],entity->camera.bmControls,entity->camera.bControlSize);
			}

			if(UVC_ENTITY_TYPE(entity) == UVC_VC_EXTENSION_UNIT){
                //printk("extension id = %x \n",entity->id);
		...
		ret = 12;
		copy_to_user(dctrl->data, data, ret);
		goto out;
    }
	
	if (set && copy_from_user(data, dctrl->data, dctrl->wLength)) {
		ret = -EFAULT;
		goto out;
	}


	ret = uvc_direct_ctrl(chain->dev,dctrl->bmRequestType,dctrl->bRequest,
			dctrl->wValue,dctrl->wIndex,data,dctrl->wLength);
   
	if (ret < 0)
		goto out;
     
	if (!set && copy_to_user(dctrl->data, data, dctrl->wLength)) {
		ret = -EFAULT;
		goto out;
	}
