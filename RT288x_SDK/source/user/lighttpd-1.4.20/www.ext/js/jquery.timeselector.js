...
(function($) {
	...
	Timeselector.prototype = {
		...
		_setTimeFromField: function(inst) {
			...
			var date = new Date(), matches, hour;
			...
			date.setHours(inst.options.hour);
			date.setMinutes(inst.options.minute);
			inst.date = date;
		},
		...
		_doKeyDown: function(event) {
			...
			switch (event.which) {
				...
				case 27: 
					...
					break;
				case 38:
					this._selectTime(inst, 'minute', 30);
					event.preventDefault();
					break;
				case 40:
					this._selectTime(inst, 'minute', -30);
		...
		_create: function() {
			...
			this._$div = $(this._generateHtml()).appendTo('body').hide()
				.on('mousedown.timeselector', '.timeselector-button', function(event) {
					...
					that._selectTime(that._inst, $button.parent('.timeselector-item').data('type'), $button.hasClass('timeselector-up') ? ($button.parent('.timeselector-item').data('type')=='hour'?1:30) : ($button.parent('.timeselector-item').data('type')=='hour'?-1:-30));
					event.preventDefault();
				})
