...
int TcpServer_Init(const char *cp,int port)
{
	int on = 1;
	int rc;
	int sockFD;
	...
	/* system can queue only 1 connection requests before */
	/* the system starts rejecting incoming requests.*/
	/*************************************************/
	/* Only 1 client can be queued */
	if((rc = listen(sockFD, 128)) < 0) {
...
int TcpWaitingClient(int sockFD,int timeoutS)
{
	...
	if((clientSocket = accept(sockFD, (struct sockaddr *)&their_addr, &sin_size)) < 0) {
	...
	if(timeoutS > 0){
		...
		setsockopt(clientSocket, IPPROTO_TCP, TCP_KEEPIDLE,&keepidle, sizeof(keepidle));
		setsockopt(clientSocket, IPPROTO_TCP, TCP_KEEPINTVL,&keepinterval, sizeof(keepinterval));
		setsockopt(clientSocket, IPPROTO_TCP, TCP_KEEPCNT,&keepcount, sizeof(keepcount));
	}
	
   
	if((rc = setsockopt(clientSocket,IPPROTO_TCP,TCP_NODELAY, (char *)&on, sizeof(on))) < 0) {
		perror("Server-setsockopt() error");
		return -1;
	}
	printf("Got connection from the client: %d %s\n",sockFD, inet_ntoa(their_addr.sin_addr));
