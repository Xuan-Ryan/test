...
#define FPRINT_STR(x) fprintf(fp, #x"=%s\n", nvram_bufget(mode, #x));
	...
	if ((RT2860_NVRAM == mode) 
		...
		FPRINT_STR(BW_Guarantee_Airtime_Ratio);
		FPRINT_STR(BW_Maximum_Airtime_Ratio);
                if(atoi(nvram_bufget(mode,"BW_Enable")))
		{
	            fprintf(fp, "BW_Ctrl_Enable=1\n");
	            fprintf(fp, "AT_Ctrl_Enable=1\n");
		}
		/*
